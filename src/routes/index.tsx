...
function FeaturedGoldSection() {
  ...
  useEffect(() => {
    fetchData("/api/v1/shop/featured-product")
      .then((res) => {
        // Support both array (new multi-item backend) and single object (legacy)
        if (Array.isArray(res) && res.length > 0) setItems(res);
        else if (res && (res.name || res.headline)) setItems([res]);
        else if (res && res.product) setItems([res]);
      })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [fetchData]);

  // Auto-advance with zoom-in animation
  useEffect(() => {
  ...
  // Normalise — item could be {name, description, imageUrl} or legacy {headline, blurb, product}
    ...
    <section className="relative py-20 md:py-24 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-5xl">
        <div className="text-center mb-10">
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/20 text-white text-xs uppercase tracking-[0.25em] font-bold backdrop-blur-sm">
            <Award className="h-3.5 w-3.5" /> Featured
          </span>
        </div>

        {/* Single featured item — zoom-in transition */}
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-center">
          <div
            className="relative rounded-[2rem] overflow-hidden shadow-2xl aspect-[4/3] bg-black/10 border border-white/30"
            style={{ transition: "transform 0.4s ease, opacity 0.4s ease", transform: zoomed ? "scale(1)" : "scale(0.92)", opacity: zoomed ? 1 : 0 }}
          >
            {imageUrl ? (
              <img src={imageUrl} alt={name} className="absolute inset-0 w-full h-full object-cover" />
            ) : (
              <div className="absolute inset-0 bg-gradient-to-br from-[#92670f]/40 to-[#5c3a05]/60 grid place-items-center">
                <Award className="h-16 w-16 text-[#f0c33d]/40" />
              </div>
            )}
          </div>
          ...
          >
            ...
            {description && <p className="mt-3 text-[#4a3309] leading-relaxed text-sm md:text-base">{description}</p>}
          </div>
        </div>
          ...
          <div className="flex justify-center gap-2 mt-8">
            {items.map((_, i) => (
              <button key={i} onClick={() => { setZoomed(false); setTimeout(() => { setIdx(i); setZoomed(true); }, 300); }}
                className={`h-2 rounded-full transition-all ${i === idx ? "w-8 bg-[#241502]" : "w-2 bg-[#241502]/30"}`} />
            ))}
          </div>
        )}
      </div>
