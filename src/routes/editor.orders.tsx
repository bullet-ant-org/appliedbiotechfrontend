...
export const Route = createFileRoute("/editor/orders")({ component: EditorOrders });
  ...
  async function updateStatus(id: string, next: string, eta?: string) {
    try {
      await fetchData(`/api/v1/payments/orders/${id}/status-update`, {
        ...
        body: JSON.stringify({ status: next, eta })
      });
      toast.success(`Order marked as ${next}`);
      loadOrders();
      setViewing((v) => v && v.id === id ? { ...v, status: next, eta } as any : v);
...
export function OrderDetailModal({ order, onClose, onUpdateStatus }: { order: Order | null; onClose: () => void; onUpdateStatus: (id: string, status: string, eta?: string) => void }) {
  const [etaDate, setEtaDate] = useState("");

  useEffect(() => {
    setEtaDate(order?.eta ? order.eta.slice(0, 10) : "");
  }, [order?.id]);
  ...
  function handleStatusClick(s: string) {
    if (!etaDate) {
      toast.error("Please set an estimated delivery date (ETA) before changing the order status");
      return;
    }
    onUpdateStatus(order!.id, s, etaDate);
  }
      ...
      footer={
        <div className="w-full space-y-3">
          <div className="flex items-center gap-2">
            <label className="text-xs font-semibold text-muted-foreground shrink-0">Estimated delivery (ETA):</label>
            <input
              type="date"
              value={etaDate}
          ...
          <div className="flex flex-wrap gap-2">
            {STATUS_OPTIONS.map((s) => (
              <button
                ...
                disabled={!etaDate}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold capitalize border transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${order.status === s ? "gradient-brand text-brand-foreground border-transparent" : "border-border hover:bg-accent"}`}
              >
                {s}
              </button>
            ))}
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="rounded-xl bg-secondary/50 p-3">
            ...
            <div className="text-sm mt-1 font-medium">{order.eta ? new Date(order.eta).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }) : "Not set"}</div>
          </div>
        </div>
