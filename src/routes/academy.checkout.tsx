...
function AcademyCheckoutPage() {
  ...
  const onSuccess = useCallback((res: any) => {
    ...
    // Hard redirect — same reasoning as shop checkout: Paystack's popup teardown can
    // interrupt an in-app SPA navigation, leaving the spinner stuck forever.
    window.location.assign(`/verify?reference=${encodeURIComponent(reference)}`);
