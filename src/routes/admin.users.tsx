...
export const Route = createFileRoute("/admin/users")({ component: () => {
  ...
  useEffect(() => {
    const getUsers = async () => {
      try {
        ...
        if (result) setUsers((result as any[]).map((u: any) => ({ ...u, id: u._id || u.id })));
      } catch (err: any) {
  ...
  async function handleCreateUser() {
    ...
    try {
      ...
      if (result) {
        ...
        if (updatedUsers) setUsers((updatedUsers as any[]).map((u: any) => ({ ...u, id: u._id || u.id })));
      }
    } catch (err: any) {
  ...
  async function handleRoleChange() {
    ...
    const newRole = roleTarget.role === "admin" ? "editor" : "admin";
    setRoleUpdating(true);
    try {
      await fetchData(`/api/v1/users/${roleTarget.id}/role-escalation`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  return <div className="space-y-6">
      ...
      <div className="overflow-x-auto pb-48">
        ...
        <tbody className="divide-y divide-border">{users.filter((u) => (String(u.fullName || "") + String(u.email || "")).toLowerCase().includes(q.toLowerCase())).map((u) => (
          <tr key={u.id} className="hover:bg-muted/40">
            ...
            <td className="pr-4"><RowMenu actions={[{ label: "Edit role", onClick: () => { setRoleTarget(u); setRoleOpen(true); } }, { label: "Reset password", onClick: () => toast.success("Reset link sent") }, { label: "Suspend", danger: true, onClick: () => setUsers((p) => p.map((x) => x.id === u.id ? { ...x, status: "Suspended" } : x)) }]} /></td>
          </tr>
        ))}
    ...
    <Modal
      open={roleOpen}
      onClose={() => { setRoleOpen(false); setRoleTarget(null); }}
      title="Change User Role"
      footer={
        <>
          <GhostBtn onClick={() => { setRoleOpen(false); setRoleTarget(null); }}>Cancel</GhostBtn>
          <PrimaryBtn onClick={handleRoleChange} disabled={roleUpdating}>
            {roleUpdating ? <Loader2 className="h-4 w-4 animate-spin" /> : `Change to ${roleTarget?.role === "admin" ? "Editor" : "Admin"}`}
          </PrimaryBtn>
        </>
      }
    >
      {roleTarget && (
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-4 rounded-xl bg-secondary/60 border border-border">
            <span className="h-10 w-10 rounded-full gradient-brand grid place-items-center text-brand-foreground text-xs font-bold shrink-0">
              {roleTarget.fullName.split(" ").map((n: string) => n[0]).slice(0, 2).join("")}
            </span>
            <div>
              <div className="font-semibold text-sm">{roleTarget.fullName}</div>
              <div className="text-xs text-muted-foreground">{roleTarget.email}</div>
            </div>
          </div>
          <p className="text-sm text-muted-foreground">
            Do you want to change <span className="font-semibold text-foreground">{roleTarget.fullName}</span>'s role from{" "}
            <span className={`font-semibold capitalize ${roleTarget.role === "admin" ? "text-brand" : "text-amber-600"}`}>{roleTarget.role}</span>{" "}
            to{" "}
            <span className={`font-semibold capitalize ${roleTarget.role === "admin" ? "text-amber-600" : "text-brand"}`}>
              {roleTarget.role === "admin" ? "editor" : "admin"}
            </span>?
          </p>
          <p className="text-xs text-muted-foreground bg-secondary/50 rounded-lg p-3 border border-border">
            {roleTarget.role === "admin"
              ? "Editors can manage content and orders but cannot manage users or system settings."
              : "Admins have full access including user management and system settings."}
          </p>
        </div>
      )}
    </Modal>
