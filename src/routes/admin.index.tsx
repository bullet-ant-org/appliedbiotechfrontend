...
import {
  Users, DollarSign, Package, GraduationCap, ShoppingCart, Heart, Download, Loader2, ArrowUpRight, ArrowDownRight
} from "lucide-react";
  ...
  useEffect(() => {
    const getOverview = async () => {
      try {
        ...
        const [users, orders, products, academy, analytics, academyMetrics] = await Promise.all([
          fetchSafe("/api/v1/users"),
          fetchSafe("/api/v1/payments/orders-ledger"),
          fetchSafe("/api/v1/shop/products"),
          fetchSafe("/api/v1/academy"),
          fetchSafe("/api/v1/analytics/metrics"),
          fetchSafe("/api/v1/academy/administration/metrics"),
        ]);
        ...
        const totalStudents = Array.isArray(academyMetrics) ? academyMetrics.length : (courseList.reduce((acc, curr) => acc + (curr.students || 0), 0));
        ...
        setMetrics({
          ...
          monthlySales: weeks,
          totalViews: (() => { const landing = analyticList.find((item) => item.page === "landing"); return landing ? landing.views : 0; })(),
        });
      } catch (err: any) {
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Recent Orders - Properly bound and responsive */}
        <div className="lg:col-span-2 bg-card rounded-2xl border border-border p-6 shadow-soft min-w-0 w-full">
          <div className="flex items-center justify-between mb-4">
            ...
            <table className="w-full text-sm min-w-[600px]">
              <thead>
                <tr className="text-left text-muted-foreground text-xs uppercase tracking-wider">
                  <th className="pr-2 py-2">Tracking ID</th>
                  <th className="px-2 py-2">Customer</th>
                  <th className="px-2 py-2">Status</th>
                  <th className="pl-2 py-2 text-right">Total</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {metrics.orders.map((o) => (
                  <tr key={o.id} className="hover:bg-muted/40">
                    <td className="pr-2 py-3 font-mono text-xs truncate max-w-[120px]">{o.id}</td>
                    <td className="px-2 py-3 truncate max-w-[180px]">{o.email}</td>
                    <td className="px-2 py-3"><StatusPill s={o.status || "Paid"} /></td>
                    <td className="pl-2 py-3 text-right font-semibold">₦{o.totalAmount.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
