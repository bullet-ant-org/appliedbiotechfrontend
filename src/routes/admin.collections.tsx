...
function AdminCollections() {
  ...
  async function handleSave() {
    ...
    if (!editing && !coverFile) return toast.error("Please upload a cover image");
    ...
    // Attach actual File object if user uploaded a new image
    if (coverFile) {
    ...
    <div className="space-y-6">
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit collection" : "New collection"}
        ...
        <div className="space-y-4">
          <ImageUpload label="Cover image" value={cover} onChange={(url, file) => { setCover(url); if (file) setCoverFile(file); }} aspect="aspect-[16/9]" />
