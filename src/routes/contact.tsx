...
import { Mail, Phone, MapPin, Send, Clock, Loader2, Check, Building2 } from "lucide-react";
import { toast } from "sonner";
import useFetch from "@/hooks/useFetch";

const DEPARTMENTS = [
  { id: "capacity", label: "Capacity Building", email: "info@appliedbiotech.ng", desc: "Training, certifications and workforce upskilling" },
  { id: "equipment", label: "Equipment Supplies", email: "sales@appliedbiotech.ng", desc: "Procurement, reagents and lab consumables" },
  { id: "consultancy", label: "Consultancy, Contracts & Grants", email: "info@appliedbiotech.ng", desc: "Grant writing, strategy and project management" },
  { id: "lab", label: "Molecular Laboratory Services", email: "lab_analyst@appliedbiotech.ng", desc: "Sequencing, diagnostics and sample analysis" },
  { id: "bio-mfg", label: "Bio-Manufacturing", email: "president@appliedbiotech.ng", desc: "Local devices, consumables and reagents" },
];
...
function ContactPage() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const targetEmail = dept.email;

    // Construct the mailto URL with pre-filled fields, routed to the chosen department
    const bodyText = `Sender Name: ${first} ${last}\nSender Email: ${senderEmail}\nDepartment: ${dept.label}\n\nMessage Content:\n${message}`;
    const mailtoUrl = `mailto:${targetEmail}?subject=${encodeURIComponent(String(subject))}&body=${encodeURIComponent(bodyText)}`;

    // Redirect to the mailto link to open the local email app
    window.location.href = mailtoUrl;

    toast.success(`Redirecting to your email client for ${dept.label}...`);
    setDone(true);
  }
            ...
            <form className="reveal rounded-2xl border border-border bg-card p-7 shadow-soft space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                <label className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5 mb-2">
                  <Building2 className="h-3.5 w-3.5" /> Which department do you want to reach?
                </label>
                <div className="grid gap-2">
                  {DEPARTMENTS.map((d) => (
                    <label
                      key={d.id}
                      className={`flex items-start gap-3 rounded-lg border px-3 py-2.5 cursor-pointer transition-colors ${department === d.id ? "border-brand bg-brand/5" : "border-input hover:bg-accent"}`}
                    >
                      <input
                        type="radio"
                        name="department"
                        value={d.id}
                        checked={department === d.id}
                        onChange={() => setDepartment(d.id)}
