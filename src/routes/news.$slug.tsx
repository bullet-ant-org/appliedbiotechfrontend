    ...
    try {
      ...
      if (res && (Array.isArray(res) || Array.isArray(res.data))) {
        ...
        const normalized = list.map((n: any) => {
          // body can be a string, rich-text object, or array of sections
          let bodyText = "";
          const raw = n.body ?? n.newsBody ?? n.content ?? "";
          if (typeof raw === "string") {
            bodyText = raw;
          } else if (Array.isArray(raw)) {
            bodyText = raw.map((s: any) => (typeof s === "string" ? s : s?.content || s?.text || "")).join("\n\n");
          } else if (raw && typeof raw === "object") {
            // Rich text object — extract text from sections/blocks
            const sections = raw.sections || raw.blocks || raw.content || [];
            if (Array.isArray(sections)) {
              bodyText = sections.map((s: any) => (typeof s === "string" ? s : s?.content || s?.text || "")).join("\n\n");
          ...
          return {
            ...
            body: bodyText,
            cover: n.coverImage || n.image || n.cover || "",
            date: new Date(n.createdAt || n.date).toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" })
          };
        });
